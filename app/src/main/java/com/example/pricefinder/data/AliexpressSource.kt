...
class AliexpressSource(
    ...
    override val source = Source.ALIEXPRESS

    override suspend fun search(query: String): List<PriceItem> {
        val q = URLEncoder.encode(query, "UTF-8")
        val url = "https://aliexpress.ru/wholesale?SearchText=$q"
        ...
        val titleRegex = Regex("\"(?:title|subject|productTitle)\"\\s*:\\s*\"([^\"]{3,120})\"")
        val priceRegex = Regex("\"(?:minPrice|salePrice|displayPrice|price)\"\\s*:\\s*\"?(\\d{2,8})")
